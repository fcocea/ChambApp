...
; handleRefresh?: () => void; }) => {
  ...
  const handleApplyService = useCallback(async () => {
    setLoading(true);
    const response = await fetch(`${API_ENDPOINT}/advertisements/${data?.ad_id}/apply`, {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      ...
      Alert.alert("Error", data?.detail || "Error al postular el servicio");
    }
        ...
        >
          <View className="self-center flex flex-row items-center justify-center gap-2">
            {loading
              ...
              : <FileUser size={18} color="#ffffff" />}
            <Text className="text-white text-center text-base font-medium w-fit">Postular</Text>
          </View>
        </TouchableOpacity>
      </View>
    </BottomSheetView>
